...
class Core_Utils_Utils {
    ...
    static function existFolder($path, $nameFolder) {
        if (!is_dir($path)) {
            mkdir($path . '/' . $nameFolder, 0777, true);
        }
    }

    /**
     * 
     * determina si existe cabecera HTTP_X_FILENAME
     * @param array $SERVER
     */
    static function existHTTP_X_FILENAME($SERVER) {
        if ($SERVER) {
            return $SERVER;
        } else {
    ...
    /**
     * Extrae la extencion de un archivo
     * @param string $namefile
     */
    static function extractExt($namefile) {
        return substr($namefile, strrpos($namefile, '.') + 1);
    }
    ...
    static function validateExtIpaApk($ext) {
        $arrayExt = array('apk', 'APK', 'ipa', 'IPA');
        if (!in_array($ext, $arrayExt)) {
            throw new Exception('Extencion incorrecta');
        }
    }

    static function listedDirectory($path) 
    {        
